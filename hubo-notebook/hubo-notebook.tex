...
\center
% Headings
\textsc{\LARGE Georgia Ins Technology}\\[1.5cm]
...
\section*{March 8}
\subsection*{Clock and Timer stuff (Cross-platform)}
Nanosecond resolution for Linux and Mac. Microsecond resolution for Windows
\begin{itemize}
  \item Function to Use
    \begin{itemize}
    \item Linux: clock\_getTime(CLOCK\_MONOTONIC, struct timespec* t)
    \item Windows: QueryPerformanceCounter() and QueryPerformanceFrequency()
    \item Mac: mach\_absolute\_time()
    \end{itemize}
  \item Implementation
    \begin{lstlisting}
    inline double getTime() {
        ...
        uint64_t time;
        return double(*(uint64_t*)AbsoluteToNanoseconds(
                      *(AbsoluteTime*)(mach_absolute_time()))) * 1e-9;
p#else
...
\section*{Tasks}
\begin{itemize}
\item Add ach flushes to any where where cmds get received
\item Fix "Run CRPC" button coloration.
\item Test walking again and plot landing controller influence with forces.
...
\item \textbf{DONE} Modify the impedance controller logic to act as a landing controller during single-support phase.
\item \textbf{DONE} Test and fix biped walking on the robot.
\item Get walking to a point/theta written and working.
\item Add state feedback from Walker.
\item \textbf{DONE 9/25} Clone Matt's logger and add stance to it.
...
\section*{November 21}
\subsection*{Hokuyo Stuff}
\begin{itemize}
	\item Hokuyo ip address tool: http://www.hokuyo-aut.jp/02sensor/07scanner/download/products/utm-30lx-ew/
    \item default ip: 192.168.1.47
\end{itemize}


\section*{November 20}
\subsection*{Drill Grasping Parameters}
\begin{itemize}
	\item Height above table: 11.8 cm
	\item Angle w.r.t Drill about z: -8 degrees (ie. Angle b/t local x and drill bit is 8 degrees rotated negative about z
	\item Angle about local y: +4 degrees
\end{itemize}

\section*{November 03}
\subsection*{Hubo-ach Audit}
\begin{itemize}
  \item Use clock\_monotonic, NOT clock\_realtime, because clock\_realtime can be discontinuous.
  \item Position is constant for two ticks. Most likely encoder reading error, since the motors probably can't accelerate that fast.
  \item IMU off: Delay b/t cmd and pos = 25ms. dt not okay.
  \item IMU  on: Delay b/t cmd and pos = 130ms. dt okay.
  \item Discontinuities in ref from state and pos.
  \item Delay in dt.
\end{itemize}

\section*{October 23}
\subsection*{DRC Prep Meeting}
Locomotion
\begin{itemize}
  \item Static Walking
	\begin{itemize}
	  \item COM nudging using ZMP feedback
	  \item ZMP Filtering
	  \item ZMP Preview Control
	\end{itemize}
  \item Robust Dynamic Walking
    \begin{itemize}
      \item Implement 3-state impedance controller with averaging between feet
    \end{itemize}
  \item Static Balancing (Very general, ie. no assumptions)
\end{itemize}

\section*{October 11}
\subsection*{Humanoids Drill Demo}
\begin{itemize}
  \item sudo hubo-ach start drc
  \item cd ~/other/walking-controllers/build
  \item sudo ./read-trajectory -n ../traj/drill\_traj.txt -f 200 -i -c1
  \item sudo ./handctrl r open
  \item sudo ./handctrl r close
  \item sudo ./read-trajectory -n ../trajs/drill\_traj.txt -f 200 -i
  \item sudo ./handctrl r open
  \item sudo ./handctrl r close
  \item sudo ./read-trajectory -z
\end{itemize}

\subsection*{Teleop GUI}
\begin{itemize}
  \item a-price/drchubo develop branch
  \item golems/hubo\_gt\_ros nullspace branch
  \item roslaunch fullbody-teleop
  \item roslaunch hubo\_state
  \item fixed frame: /planRightFoot
  \item robot: robot\_planning\_description (for Robot Description)
  \item robot: robot\_description (for Robot state)
  \item pose: topic-/rh\_pose, axes length-.2, axes radius-.01
  \item interactive marker: topic-joint\_controls/update
\end{itemize}
For perception
\begin{itemize}
  \item export ROS\_MASTER\_URI=http://192.168.1.46:11311/
  \item git clone https://github.com/hubo/hubo\_head\_description
  \item git checkout origin/teleop -- hubo\_drc\_vision/launch/hubo\_remote\_rgbd.launch (in hubo\_gt\_ros)
  \item roslaunch hubo\_drc\_vision hubo\_remote\_rgbd.launch
  \item roslaunch hubo\_drc\_vision hubo\_head.launch
\end{itemize}
On the Perception Laptop
\begin{itemize}
  \item roslaunch hubo\_launch sensor\_head\_controller.launch
  \item Pan Angle to -180 and hit "Pan" in the HeadControlPanel
\end{itemize}

\section*{October 09}
\subsection*{Status}
\begin{itemize}
  \item Biped walking is back to where it was with Hubo2+. The parameter set is as follows:
    \begin{itemize}
      \item Biped Parameters \& CRPC Parameters
    \end{itemize}
\end{itemize}
        \includegraphics[width=8cm]{resources/crpcParams-no-backpack-with-battery} $\quad$
        \includegraphics[width=8cm]{resources/bipedParams-no-backpack-with-battery}
...
\subsection*{Todo}
\begin{itemize}
  \item Changed convergence norm in Walker.cpp (\textbf{DONE}) and tested
  ...
  \item Fix waist control in rviz panel. (\textbf{DONE}) by grey
